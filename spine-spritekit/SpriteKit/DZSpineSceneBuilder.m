...
#else
				rect = NSRectToCGRect(NSRectFromString(override[@"rectInAtlas"]));
#endif
            }
        }
...
#else
				string = NSStringFromRect(NSRectFromCGRect(rect));
#endif
                NSLog(@"sprite: texture missing for %s atlas:%@ rect:%@", attachment->name, atlasName, string);
            }
        }
    }
    return texture;
...
- (void) setTextureName:(NSString *) textureName rect:(CGRect) rect forAttachmentName:(NSString *) attachmentName
{
    if ( textureName && attachmentName ) {
		...
		string = NSStringFromRect(NSRectFromCGRect(rect));
#endif
        self.maps.mapOverrideAttachmentToTexture[attachmentName] = @{ @"textureName" : textureName, @"rectInAtlas" : string };
